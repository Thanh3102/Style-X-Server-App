...
export class CartService {
  ...
  async syncCart(
    ...
    try {
      ...
      const userCart = await this.findUserCardById(userId);
      const guestCart = await this.findGuestCartById(guestCartId);
      ...
      await this.deleteGuestCart(guestCart.id);
  ...
  async checkItemQuantityAvaiable(
    ...
    const { avaiable } =
      await this.productService.getVariantInventoryStock(variantId);
